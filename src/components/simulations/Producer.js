...
function Producer() {
  ...
  function scrollToBottom() {
    var scrollableDiv = document.getElementsByClassName('scrollableDiv');
  ...
  const produceItem = () => {
    ...
    if (count === BUFFER_SIZE) {
      addToLogs('Buffer is full. Producer is waiting...');
      return;
    }
  ...
  const consumeItem = () => {
    if (count === 0) {
      addToLogs('Buffer is empty. Consumer is waiting...');
      return;
    }
  ...
  return (
    <div style={{ textAlign: 'center' }}>
      ...
      <button onClick={produceItem} className='custom-btn btn-7'>Produce Item</button>
      <button onClick={consumeItem} className='custom-btn btn-7'>Consume Item</button>
      <button onClick={resetState} className='custom-btn btn-7'>Reset</button>
    </div>
  );
}
