...
function Nextfit(props) {
  const initialProgress = 0;
    ...
    function manager(param){
      ...
      const url = `http://127.0.0.1:5000/Nextfit?param=${encodeURIComponent(param)}`;
        ...
        for (const update of progresses) {
          if (update === -1) {
            ...
            if (willFit) {
              ...
              setoccupancies(temp);
              setdisplayinfo(`Next Best Fit Found at Block ${index+1}`);
    ...
    function getprogresses(param){
      const url = `http://127.0.0.1:5000/Nextfitadd?param=${encodeURIComponent(param)}`;
    ...
    return (
      <div className="FCFS">
        <center><h2 style={{color:'white',fontFamily:'fantasy'}}>Next Best Fit Dynamic Memory Scheduling</h2></center>
        <br />
