...
function Worstfit(props) {
  const initialProgress = 0;
    ...
    function manager(param){
      ...
      const url = `http://127.0.0.1:5000/Worstfit?param=${encodeURIComponent(param)}`;
        ...
        for (const update of progresses) {
          if (update === -1) {
            ...
            if (willFit) {
              ...
              setoccupancies(temp);
              setdisplayinfo(`Worst Fit Found at Block ${index+1}`);
    ...
    function getprogresses(param){
      const url = `http://127.0.0.1:5000/Worstfitadd?param=${encodeURIComponent(param)}`;
    ...
    return (
      <div className="FCFS">
        <center><h2 style={{color:'white',fontFamily:'fantasy'}}>Worst Fit Dynamic Memory Scheduling</h2></center>
        <br />
